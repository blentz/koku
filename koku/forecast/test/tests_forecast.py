...
class AWSForecastTest(IamTestCase):
    ...
    def test_predict_flat(self):
        ...
        expected = []
        for n in range(0, 10):
            expected.append((dh.n_days_ago(dh.today, 10 - n), 5))

        mocked_table = Mock()
        mocked_table.objects.filter.return_value.order_by.return_value.values_list.return_value = expected
        ...
        for item in results:
            ...
            self.assertAlmostEqual(item.get("value"), 5, delta=0.0001)
            self.assertAlmostEqual(item.get("confidence_max"), 5, delta=0.0001)
            self.assertAlmostEqual(item.get("confidence_min"), 5, delta=0.0001)
            self.assertAlmostEqual(item.get("rsquared"), 1, delta=0.0001)
    ...
    def test_predict_increasing(self):
        ...
        expected = []
        for n in range(0, 10):
            expected.append((dh.n_days_ago(dh.today, 10 - n), n))
        ...
        for item in results:
            self.assertRegex(item.get("date"), r"\d{4}-\d{2}-\d{2}")
            self.assertGreaterEqual(item.get("value"), 0)
            self.assertGreaterEqual(item.get("confidence_max"), 0)
